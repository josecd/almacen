...
export class ContactsDetailsComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this.Form = new UntypedFormGroup({
          ...
          this.FormUpdate = this._formBuilder.group({
            ...
            categoria:new UntypedFormControl('',[Validators.required]),
            caducidad:new UntypedFormControl('',[Validators.required]),
            fecha_entrada:new UntypedFormControl(''),
            fecha_salida:new UntypedFormControl('',),
            nombre_entrada:new UntypedFormControl('default'),
            status:new UntypedFormControl(1,),
          })
        ...
        this._contactsService.contact$
            ...
            .subscribe((contact: any) => {
                ...
                this._contactsService.getHistorial(contact.id).subscribe(res=>{
                    let info: any;
                    info = res.map((e: any) => ({
                        id: e.payload.doc.id,

                        fecha1:moment(e.payload.doc.fecha).format("DD-MM-YYYY"),
                        fecha2:new Date(moment(e.payload.doc.fecha).format("DD-MM-YYYY")) ,
                        ...e.payload.doc.data(),
                    }));
                    this.historial = info
        ...
        bajaArticulo(): void
    {
        // Open the confirmation dialog
        const confirmation = this._fuseConfirmationService.open({
            title  : '¿Desea dar de baja el articulo?',
            message: '¡Esta acción no se puede deshacer!',
            actions: {
                confirm: {
                    label: 'Eliminar'
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if ( result === 'confirmed' )
            {
                // Get the current contact's id
                const id = this.contact.id;
                this._contactsService.bajaArticulo(id)
            }
        });
