...
export class ContactsService {
    ...
    getContacts(): Observable<Contact[]> {
        return this.afs
            .collection('articulos', ref => ref.where('status',"==",1))
            .snapshotChanges()
            .pipe(
                tap((contacts: any) => {
    ...
    updateContact(id,contact,precio,nombre): Observable<any> {
        ...
        console.log(id);
         ...
         return this.contacts$.pipe(
            take(1),
            switchMap((contacts) => this.afs.collection('articulos').doc(id).update({
                nombre: contact.nombre,
                precio: contact.precio
            }).then((res: any) => {
                let print
                if (contact.precio == precio && contact.nombre == nombre) {
                    ...
                    if (contact.precio != precio && contact.nombre != nombre) {
                        print = 'Se modifico nombre y precio'
                    }else{
                        if (contact.precio != precio) {
                            print = 'Se modifico precio'
                        }
    ...
    getArticulos(): Observable<any> {
        return this.afs.collection('articulos').snapshotChanges();
    }
    ...
    createArticulo(data) {
        this.afs
            ...
            .add({
                ...
                caducidad: moment(data.caducidad).format("MM-DD-YYYY"),
    ...
    getHistorial(id) {
        return this.afs.collection('articulos').doc(id).collection('historial', ref => ref.orderBy('fechaTime', 'desc')) .snapshotChanges();
    }
        /**
   * Get contacts
   */
    getArticulosObs() {
    this.afs.collection('articulos', ref => ref.where('status',"==",1) ).snapshotChanges().subscribe(res=>{
        let info: any;
        info = res.map((e: any) => ({
            id: e.payload.doc.id,
            ...e.payload.doc.data(),
   ...
   */
    bajaArticulo(id) {
        return this.afs.collection('articulos').doc(id).update({
            fecha_salida:moment().format("MM-DD-YYYY")
        }).then((res:any)=>{
            console.log(id);
            return this.afs
                    .collection('articulos')
